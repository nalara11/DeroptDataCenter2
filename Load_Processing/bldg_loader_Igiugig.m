...
filename = 'EAF renewables.xlsx';

% if (site_location == 0)
%     filename = 'EAF renewables.xlsx';
% 
%     solar = xlsread(filename, 'E6:E8766');  % Adjust range if needed
% 
%     wind = xlsread(filename, 'D6:D8766');  % Adjust range if needed
% end
% 
% if (site_location == 1)
%     filename = 'Renewable resource data.xlsx';
%     solar = xlsread(filename, 'Ashburn, VA', 'C5:C8765'); 
%     wind  = xlsread(filename, 'Ashburn, VA', 'D5:D8765'); 
% end
% if (site_location == 2)
%     filename = 'Renewable resource data.xlsx';
%     solar = xlsread(filename, 'Dallas, TX', 'C5:C8765'); 
%     wind  = xlsread(filename, 'Dallas, TX', 'D5:D8765'); 
% end
% if (site_location == 3)
%     filename = 'Renewable resource data.xlsx';
%     solar = xlsread(filename, 'Phoenix, AZ', 'C5:C8765'); 
%     wind  = xlsread(filename, 'Phoenix, AZ', 'D5:D8765'); 
% end
% if (site_location == 4)
%     filename = 'Renewable resource data.xlsx';
%     solar = xlsread(filename, 'San Jose, CA', 'C5:C8765'); 
%     wind  = xlsread(filename, 'San Jose, CA', 'D5:D8765'); 
% end
if (site_location == 0)
    filename = 'EAF renewables.xlsx';
