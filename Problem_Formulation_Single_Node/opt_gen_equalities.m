...
Constraints = [Constraints
    (sum(var_util.import,2) + sum(var_legacy_diesel.electricity,2) + sum(var_pv.pv_elec,2) + sum(var_wind.wind_elec,2) + sum(var_ees.ees_dchrg,2) + sum(var_lees.ees_dchrg,2) + sum(var_rees.rees_dchrg,2) + sum(var_ldg.ldg_elec,2) + sum(var_legacy_diesel_binary.electricity,2) + sum(var_lbot.lbot_elec,2) + sum(var_run_of_river.electricity,2) + sum(var_pem.elec,2)  + sum(var_ror_integer.elec,2) + sum(var_wave.electricity,2) + sum(var_sofc.sofc_electricity,2) ... %%%Production
    ==...
...
%hydrogen_load = ones(8760, 1);

%% Chemical ennergy conversion balance - Hydrogen
if ~isempty(el_v) || ~isempty(rel_v) || ~isempty(el_binary_v)
    Constraints = [Constraints
      % (sum(var_rel.rel_prod,2) + sum(var_el.el_prod,2) + sum(var_el_binary.el_prod,2) + sum(var_rel.rel_prod_wheel,2) + sum(var_el.el_prod_wheel,2) + sum(var_h2es.h2es_dchrg,2) == sum(var_pem.elec./0.5,2) + sum(var_ldg.ldg_hfuel,2) + sum(var_ldg.db_hfire,2) + sum(var_boil.boil_hfuel,2) + sum(var_h2es.h2es_chrg,2) + var_hrs.hrs_supply + var_h2_inject.h2_inject + var_h2_inject.h2_store + var_sofc.sofc_electricity./sofc_efficiency + hydrogen_load):'Hydrogen Balance'];

       (sum(var_rel.rel_prod,2) + sum(var_el.el_prod,2) + sum(var_el_binary.el_prod,2) + sum(var_rel.rel_prod_wheel,2) + sum(var_el.el_prod_wheel,2) + sum(var_h2es.h2es_dchrg,2) == sum(var_pem.elec./0.5,2) + sum(var_ldg.ldg_hfuel,2) + sum(var_ldg.db_hfire,2) + sum(var_boil.boil_hfuel,2) + sum(var_h2es.h2es_chrg,2) + var_hrs.hrs_supply + var_h2_inject.h2_inject + var_h2_inject.h2_store + var_sofc.sofc_electricity/sofc_efficiency):'Hydrogen Balance'];
end
