...
if wind_on 
    ...
    %%%PV Generation to meet building demand (kWh)
    var_wind.wind_elec = sdpvar(T,size(wind_v,2),'full'); %%% PV Production sent to the building

    %%%Size of installed System (kW)
    var_wind.wind_adopt= sdpvar(1,size(wind_v,2),'full'); %%%PV Size

    %%%PV Cost

    Objective = Objective ...
        + sum(M*wind_mthly_debt.*wind_cap_mod.*var_wind.wind_adopt) ... %%%PV Capital Cost ($/kW installed)
        + sum(sum((wind_v(3,:).*day_multi).*(var_wind.wind_elec)));  %%%PV O&M Cost ($/kWh generated)    
%         + sum(sum((wind_v(3,:).*day_multi).*(var_wind.wind_nem))); %%%PV O&M Cost ($/kWh generated)
%         + wind_v(3)*(sum(sum(repmat(day_multi,1,K).*(var_wind.wind_elec + var_wind.wind_nem + wind_wholesale))) ); %%%PV O&M Cost ($/kWh generated)
    ...
    var_wind.wind_adopt=zeros([1 1]);
    var_wind.wind_elec=zeros([T 1]);
    var_rees.rees_adopt=zeros(1,1);
    var_rees.rees_chrg=zeros(T,1);
    var_rees.rees_dchrg=zeros(T,1);
    var_rees.rees_soc=zeros(T,1);
...
%%%Seperate 
if sofc_on
    var_sofc.adopted_sofc = intvar(1,1,'full');
    %operational_decisions = 12 %if monthly on/off decision
    operational_decisions = 52
    %operational_decisions = 52 %if weekly on/off decision
    %%% operational_decisions = 365 %if daily on/off decisions
    var_sofc.sofc_operational_state =intvar(operational_decisions,1,'full');
    var_sofc.sofc_electricity = sdpvar(T,1,'full');
    Objective = Objective ...
        + var_sofc.adopted_sofc.*sofc_cost_per_stack ... %Capital costs
        %+ sum(var_sofc.sofc_electricity).*(sofc_maintenance_cost + ng_cost./sofc_efficiency); %Variable O&M
        + sum(var_sofc.sofc_electricity).*(sofc_maintenance_cost); %Variable O&M
else
    var_sofc.sofc_electricity = zeros(T,1);
end
