...
if pv_on
    ...
    %pv_v1=[1491.805; 1; 0*15.845];
    pv_v1=[885.39; 1; 0*15.845];   
...
if wind_on
    ...
    wind_v1=[1407.953; 1; 0*29.264];   
    
    %wind_v2=[0.7*(2300+216); 0.2 ; 0.001];
    
    %wind_v = [wind_v1 wind_v2];
     wind_v = [wind_v1];
    
%     %%%How wind capital cost is modified for different types of buildings
%     wind_cap_mod = [2/2.65 %%%Commercial/industrial
%         2.65/2.65]; %%%Residential
%     
    %%%Financial Aspects - Wind
    wind_fin = [0; ... %%%Scaling linear factor - Based on Lazards cost of electricity
        5; ... %%%MACRS Schedule
        1]; ... %%%ITC Benefit
        
    wind_fin = repmat(wind_fin,1,size(wind_v,2));
...
if ees_on
    ...
    % ees_v = [];
    %ees_v=[1599.717/4; 0; 0; 0.1; 0.95; 0.25; 0.25; .90; .90; .995];
    ees_v=[226.436; 0; 0; 0.1; 0.95; 0.25; 0.25; .90; .90; .995];
    
    %%%How pv capital cost is modified for different types of buildings
    %ees_cap_mod = [575/830 %%%Commercial/industrial
        %830/830]; %%%Residential
     ees_cap_mod = [1]; %%%All commercial
...
if h2es_on
    ...
    F = [22;0.001;0.001;0.01;1;1;1;1;1;1];
    h2es_cap = h2es_v(1);
else
    h2es_v = [];
end
...
if sofc_on
    ...
    %%% (1) Capital Cost ($/kWh installed)
    %%% (2) O&M ($/kWh generated)
    %%% (3) Max Production (Production capacity of individual fuel cell in
    %%% kWh)
    %%% (4) Efficiency (kWh of Hydrogen Potential Energy/kWh Electrical Energy)
    sofc_v = [sofc_cost_per_stack; sofc_maintenance_cost; sofc_capacity; sofc_efficiency]
else
end
%% Building space
